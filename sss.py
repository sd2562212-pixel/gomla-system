...
# إعداد واجهة النظام السحابي فائق الحداثة 2026
st.set_page_config(page_title="منظومة الإدارة المالية الذكية v2026", layout="centered")
...
st.markdown("""
    ...
    .metric-card {
        ...
        padding: 20px; border-radius: 14px; border: 1px solid #334155;
        box-shadow: 0 10px 15px -3px rgba(0,0,0,0.3); margin-bottom: 15px;
    }
    .alert-card {
...
conn = sqlite3.connect('gomla_modern_v2026_system.db', check_same_thread=False)
cursor = conn.cursor()
...
if not st.session_state.logged_in:
    st.markdown("<h1 style='text-align: center; color: #2563EB; font-weight: 800; margin-bottom:10px;'>📊 منظومة الجملة الذكية v2026</h1>", unsafe_allow_html=True)
...
# --- 2. لوحة التحكم الاحترافية (بعد الدخول الصحيح) ---
    ...
    # --- ميزة الجرد واللوحة الرقمية الذكية (Live Metrics) ---
    ...
    with m3:
        st.metric(label="💰 صافي أرباح المحل التقديرية", value=f"{total_profits:,.1f} ج.م")

    # --- ميزة التنبيهات الذكية لنقص المخزن ---
    cursor.execute("SELECT name, stock FROM products WHERE user_email=? AND stock <= 5", (user_email,))
    low_stock_items = cursor.fetchall()
    if low_stock_items:
        st.markdown("<br>", unsafe_allow_html=True)
        for item in low_stock_items:
            st.markdown(f"<div class='alert-card'>⚠️ <b>تنبيه مخزن:</b> صنف (<b>{item[0]}</b>) أوشك على النفاد! الكمية الحالية: {item[1]} قطعة فقط.</div>", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # تبويبات النظام المنظمة والمحدثة
    ...
    with tab1:
        ...
        st.markdown("---")
        st.subheader("📋 جدول إكسيل التفاعلي لجرد السلع وتحديث أسعار البيع اليومية")
        st.caption("💡 لتعديل سعر البيع أو كمية أي صنف، اكتب الرقم الجديد مباشرة في خانته بالجدول واضغط تم/Go بالآيفون.")
